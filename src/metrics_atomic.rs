...
impl AtomicMetrics {
    ...
    pub fn record_background_task(&self, _task: &str, status: &str, duration: f64) {
        self.background_task_runs_total
            .fetch_add(1, Ordering::Relaxed);
        ...
        if status == "error" {
            self.background_task_errors_total
                .fetch_add(1, Ordering::Relaxed);
        }
    }
    ...
    pub fn update_rocksdb_metrics(&self, memory_usage: i64, disk_usage: i64) {
        self.rocksdb_memory_usage_bytes
            .store(memory_usage, Ordering::Relaxed);
        self.rocksdb_disk_usage_bytes
            .store(disk_usage, Ordering::Relaxed);
    }
    ...
    pub fn snapshot(&self) -> MetricsSnapshot {
        MetricsSnapshot {
            ...
            request_duration_seconds_total: self
            ...
            rocksdb_operation_duration_seconds_total: self
                .rocksdb_operation_duration_seconds_total
            ...
            background_task_duration_seconds_total: self
                .background_task_duration_seconds_total
                .load(Ordering::Relaxed) as f64
                / 1_000_000_000.0,
...
pub fn encode_atomic_metrics(
    ...
    writeln!(
        buffer,
        "# HELP queueber_requests_total Total number of requests"
    )?;
    ...
    writeln!(
        buffer,
        "# HELP queueber_request_duration_seconds_total Total request duration in seconds"
    )?;
    writeln!(
        buffer,
        "# TYPE queueber_request_duration_seconds_total counter"
    )?;
    writeln!(
        buffer,
        "queueber_request_duration_seconds_total {}",
        snapshot.request_duration_seconds_total
    )?;

    writeln!(
        buffer,
        "# HELP queueber_queue_size Current number of available items"
    )?;
    ...
    writeln!(
        buffer,
        "# HELP queueber_queue_depth Total queue depth (available + leased items)"
    )?;
    ...
    writeln!(
        buffer,
        "# HELP queueber_lease_expirations_total Total number of lease expirations"
    )?;
    writeln!(buffer, "# TYPE queueber_lease_expirations_total counter")?;
    writeln!(
        buffer,
        "queueber_lease_expirations_total {}",
        snapshot.lease_expirations_total
    )?;

    writeln!(
        buffer,
        "# HELP queueber_failed_requests_total Total number of failed requests"
    )?;
    writeln!(buffer, "# TYPE queueber_failed_requests_total counter")?;
    writeln!(
        buffer,
        "queueber_failed_requests_total {}",
        snapshot.failed_requests_total
    )?;

    writeln!(
        buffer,
        "# HELP queueber_rocksdb_operations_total Total number of RocksDB operations"
    )?;
    writeln!(buffer, "# TYPE queueber_rocksdb_operations_total counter")?;
    writeln!(
        buffer,
        "queueber_rocksdb_operations_total {}",
        snapshot.rocksdb_operations_total
    )?;

    writeln!(
        buffer,
        "# HELP queueber_rocksdb_operation_duration_seconds_total Total RocksDB operation duration in seconds"
    )?;
    writeln!(
        buffer,
        "# TYPE queueber_rocksdb_operation_duration_seconds_total counter"
    )?;
    writeln!(
        buffer,
        "queueber_rocksdb_operation_duration_seconds_total {}",
        snapshot.rocksdb_operation_duration_seconds_total
    )?;

    writeln!(
        buffer,
        "# HELP queueber_rocksdb_memory_usage_bytes RocksDB memory usage in bytes"
    )?;
    writeln!(buffer, "# TYPE queueber_rocksdb_memory_usage_bytes gauge")?;
    writeln!(
        buffer,
        "queueber_rocksdb_memory_usage_bytes {}",
        snapshot.rocksdb_memory_usage_bytes
    )?;

    writeln!(
        buffer,
        "# HELP queueber_rocksdb_disk_usage_bytes RocksDB disk usage in bytes"
    )?;
    writeln!(buffer, "# TYPE queueber_rocksdb_disk_usage_bytes gauge")?;
    writeln!(
        buffer,
        "queueber_rocksdb_disk_usage_bytes {}",
        snapshot.rocksdb_disk_usage_bytes
    )?;

    writeln!(
        buffer,
        "# HELP queueber_background_task_runs_total Total number of background task runs"
    )?;
    writeln!(buffer, "# TYPE queueber_background_task_runs_total counter")?;
    writeln!(
        buffer,
        "queueber_background_task_runs_total {}",
        snapshot.background_task_runs_total
    )?;

    writeln!(
        buffer,
        "# HELP queueber_background_task_duration_seconds_total Total background task duration in seconds"
    )?;
    writeln!(
        buffer,
        "# TYPE queueber_background_task_duration_seconds_total counter"
    )?;
    writeln!(
        buffer,
        "queueber_background_task_duration_seconds_total {}",
        snapshot.background_task_duration_seconds_total
    )?;

    writeln!(
        buffer,
        "# HELP queueber_background_task_errors_total Total number of background task errors"
    )?;
    writeln!(
        buffer,
        "# TYPE queueber_background_task_errors_total counter"
    )?;
    writeln!(
        buffer,
        "queueber_background_task_errors_total {}",
        snapshot.background_task_errors_total
    )?;
