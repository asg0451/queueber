...
impl RetriedStorage<Storage> {
    async fn run_blocking_on_pool<T: Send + 'static>(
        ...
        let res: Result<T> = tokio::task::spawn_blocking(f)
            .await
            .map_err(Into::<Error>::into)?;
        res
    }

    async fn busy_retry_with_state<T, O, Fut, F>(
        name: &str,
        state: std::sync::Arc<std::sync::Mutex<RetryState<O>>>,
        f: F,
    ) -> Result<T>
    where
        O: Send,
        ...
        F: Fn(std::sync::Arc<std::sync::Mutex<RetryState<O>>>) -> Fut + Send + Sync + Clone,
    {
        let state_ref = std::sync::Arc::clone(&state);
        let f_outer = f.clone();
        with_rocksdb_busy_retry_async(name, move || {
            let state_inner = std::sync::Arc::clone(&state_ref);
            let f_cloned = f_outer.clone();
            async move { f_cloned(state_inner).await }
        })
        .await
    }
    ...
    pub async fn add_available_item_from_parts(
        ...
        type Owned = (std::sync::Arc<[u8]>, std::sync::Arc<[u8]>);
        let inner = std::sync::Arc::clone(&self.inner);
        let state = std::sync::Arc::new(std::sync::Mutex::new(RetryState::<Owned> {
            attempt: 0,
            owned: None,
        }));

        Self::busy_retry_with_state("add_available_item_from_parts", state.clone(), move |st| {
            ...
            async move {
                ...
                let res = if let Some(owned) = { st.lock().unwrap().owned.as_ref().cloned() } {
                    Self::run_blocking_on_pool(move || {
        ...
        I: IntoIterator<Item = (&'a [u8], (&'a [u8], u64))>,
    {
        ...
        type Owned = std::sync::Arc<Vec<OwnedItem>>;
        let state = std::sync::Arc::new(std::sync::Mutex::new(RetryState::<Owned> {
            attempt: 0,
            owned: None,
        }));

        Self::busy_retry_with_state("add_available_items_from_parts", state.clone(), move |st| {
            let inner_ref = std::sync::Arc::clone(&inner);
            ...
            async move {
                ...
                let res = if let Some(owned_ref) = { st.lock().unwrap().owned.as_ref().cloned() } {
                    Self::run_blocking_on_pool(move || {
                        let iter = owned_ref
                            .iter()
    ...
    pub async fn remove_in_progress_item(&self, id: &[u8], lease: &Lease) -> Result<bool> {
        ...
        Self::busy_retry_with_state("remove_in_progress_item", state.clone(), move |st| {
            let inner_ref = std::sync::Arc::clone(&inner);
            async move {
                {
                    let mut guard = st.lock().unwrap();
                    if guard.attempt > 0 && guard.owned.is_none() {
                        guard.owned = Some(std::sync::Arc::<[u8]>::from(id.to_vec()));
                    }
                }

                let out = if let Some(id_arc) = { st.lock().unwrap().owned.as_ref().cloned() } {
                    Self::run_blocking_on_pool(move || {
