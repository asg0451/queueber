...
//         bogus: bool,
//     },
//     // Failure injection: invalid inputs
//     InvalidExtendZero,
...
//     #[test]
//     fn randomized_rpc_ops(op_sequences in proptest::collection::vec(proptest::collection::vec(arb_op(), 15..35), 2..4)) {
...
//                                     }
//                                     let _ = add.send().promise.await; // ignore errors
//                                 }).await;
//                             }
...
//                                             }
//                                         },
//                                         Err(_e) => { /* failure injection path may trigger errors */ }
//                                     }
//                                 }).await;
//                             }
...
//                                         req.set_lease_validity_secs(lease_secs);
//                                     }
//                                     let _ = ext.send().promise.await; // ignore result
//                                 }).await;
//                             }
//                             Op::Remove { bogus } => {
//                                 let id = uuid::Uuid::now_v7().into_bytes().to_vec();
//                                 let lease = if bogus { uuid::Uuid::now_v7().into_bytes().to_vec() } else { let guard = current_lease.lock().unwrap(); guard.clone().unwrap_or_else(|| uuid::Uuid::now_v7().into_bytes().to_vec()) };
//                                 let _ = with_client(addr_this, |client| async move {
...
//                                         req.set_lease(&lease);
//                                     }
//                                     let _ = rm.send().promise.await; // ignore result
//                                 }).await;
//                             }
...
//                                         let mut req = ext.get().init_req();
//                                         req.set_lease(&lease);
//                                         req.set_lease_validity_secs(0); // invalid per server invariant
//                                     }
//                                     let _ = ext.send().promise.await; // expect Err sometimes
//                                 }).await;
//                             }
...
//                                     }
//                                     let _ = poll.send().promise.await; // expect Err
//                                 }).await;
//                             }
//                         }
//                     }
